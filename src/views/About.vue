<template>
    <div class="container">
        ...
        <article class="div-esquema">
            ...
            <div>
                <area-chart :data="movimientos" v-if="esquema" width="700px"></area-chart>
            </div>
        </article>
    </div>
</template>
        ...
        methods: {
            ...
            buscarMovimiento(_id) {
                this.axios.get(`/movimientoComida/${_id}`).then((response) => {
                    ...
                    console.log(this.buscarMovimientos)
                    ...
                    this.buscarMovimientos.forEach(movimiento => {
                        ...
                        if (movimiento.accion === 'creado') {
                            this.$set(this.movimientos, fecha, movimiento.valor);
                        }
                        // Si la accion es suma
                        if (movimiento.accion == 'suma') {
                            // y ya existia la posicion fecha en this.movimientos significa que ya existe esa comida y le sumamos el nuevo amount que seria la suma del dia
                            ...
                            // si existe total anterior significa que es un dia diferente, entonces le restamos al total anterior
                            this.$set(this.movimientos, fecha, (this.movimientos[fecha_anterior] ? this.movimientos[fecha_anterior] : this.movimientos[fecha]) + amount);
                        }
